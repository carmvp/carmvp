...
;$(function($){
	...
	$('.C_showdialog').on('click', function(){
		var self = $(this),
			dtitle = self.attr('data-title'),
			did = self.attr('data-id'),			
			dokval = self.attr('data-okval'),
			dwidth = parseInt(self.attr('data-width')),
			dheight = parseInt(self.attr('data-height')),
			dpadding = (self.attr('data-padding') != undefined) ? self.attr('data-padding') : '',
			dcallback = self.attr('data-callback');
...
;(function($){
    //联动菜单
    $.fn.tbcate_select = function(options) {
        var settings = {
            field: 'J_cate_id',
        ...
        $.getJSON(uri, {id:pid}, function(result){
            if(result.status == '1'){
                ...
                $('<option value="'+result.data[i].id+'" fid="'+result.data[i].fid+'">'+result.data[i].name+'</option>').appendTo(self);
                }
            }
        ...
        var j = 1;
        $(document).off('change').on('change', '.J_cate_select',function(){
            var _this = $(this),
            _pid = _this.val();
			_fid = _this.find("option:selected").attr("fid");
            _this.nextAll('.J_cate_select').remove();
            if(_pid != ''){
                $.getJSON(uri, {id:_pid}, function(result){
                    if(result.status == '1'){
                        var _childs = $('<select class="J_cate_select mr10 form-sel" data-pid="'+_pid+'"><option value="-1">--'+settings.top_option+'--</option></select>')
                        for(var i=0; i<result.data.length; i++){
                            $('<option value="'+result.data[i].id+'" fid="'+result.data[i].fid+'">'+result.data[i].name+'</option>').appendTo(_childs);
                        }
                ...
                $('#'+settings.field).val(_fid);
            }else{
            	$('#'+settings.field).val(_this.find("option:selected").attr("fid"));
            }
        });
    }
})(jQuery);
...
;(function($){
    //联动菜单
    $.fn.bcate_select = function(options) {
        var settings = {
            field: 'J_bcate_id',
            top_option: lang.please_select
        };
        ...
        self.nextAll('.J_bcate_select').remove();
        $('<option value="">--'+settings.top_option+'--</option>').appendTo(self);
        $.getJSON(uri, {id:pid}, function(result){
            if(result.status == '1'){
                for(var i=0; i<result.data.length; i++){
        ...
        var j = 1;
		 $(document).on('change', '.J_bcate_select',function(){        
            var _this = $(this),
            _pid = _this.val();
            _this.nextAll('.J_bcate_select').remove();
            if(_pid != ''){
                $.getJSON(uri, {id:_pid}, function(result){
                    if(result.status == '1'){
                        var _childs = $('<select class="J_bcate_select mr10" data-pid="'+_pid+'"><option value="">--'+settings.top_option+'--</option></select>')
                        for(var i=0; i<result.data.length; i++){
                            $('<option value="'+result.data[i].id+'">'+result.data[i].name+'</option>').appendTo(_childs);
                        }
                        _childs.insertAfter(_this);
...
;(function($){
    //联动菜单
    $.fn.qcate_select = function(options) {
        var settings = {
            field: 'J_qcate_id',
            top_option: lang.please_select
        };
        ...
        self.nextAll('.J_qcate_select').remove();
        $('<option value="">--'+settings.top_option+'--</option>').appendTo(self);
        $.getJSON(uri, {id:pid}, function(result){
            if(result.status == '1'){
                for(var i=0; i<result.data.length; i++){
		...
		$(document).off('change').on('change', '.J_qcate_select',function(){	
            var _this = $(this),
            _pid = _this.val();
            _this.nextAll('.J_qcate_select').remove();
            if(_pid != ''){
                $.getJSON(uri, {id:_pid}, function(result){
                    if(result.status == '1'){
                        var _childs = $('<select class="J_qcate_select mr10" data-pid="'+_pid+'"><option value="">--'+settings.top_option+'--</option></select>')
                        for(var i=0; i<result.data.length; i++){
                            $('<option value="'+result.data[i].id+'">'+result.data[i].name+'</option>').appendTo(_childs);
                        }
                        _childs.insertAfter(_this);
...
;(function($){
    //联动菜单
    $.fn.zcate_select = function(options) {
        var settings = {
            field: 'J_cate_id',
            top_option: lang.please_select
        };
        ...
        self.nextAll('.J_zcate_select').remove();
        $('<option value="">--'+settings.top_option+'--</option>').appendTo(self);
        $.getJSON(uri, {id:pid}, function(result){
            if(result.status == '1'){
                for(var i=0; i<result.data.length; i++){
		...
		$(document).off('change').on('change', '.J_zcate_select',function(){	
            var _this = $(this),
            _pid = _this.val();
            _this.nextAll('.J_zcate_select').remove();
            if(_pid != ''){
                $.getJSON(uri, {id:_pid}, function(result){
                    if(result.status == '1'){
                        var _childs = $('<select class="J_zcate_select mr10" data-pid="'+_pid+'"><option value="">--'+settings.top_option+'--</option></select>')
                        for(var i=0; i<result.data.length; i++){
                            $('<option value="'+result.data[i].id+'">'+result.data[i].name+'</option>').appendTo(_childs);
                        }
                        _childs.insertAfter(_this);
...
;(function($){
    //联动菜单
    $.fn.jcate_select = function(options) {
        var settings = {
            field: 'J_jcate_id',
            top_option: lang.please_select
        };
        ...
        self.nextAll('.J_jcate_select').remove();
        $('<option value="">--'+settings.top_option+'--</option>').appendTo(self);
        $.getJSON(uri, {id:pid}, function(result){
            if(result.status == '1'){
                for(var i=0; i<result.data.length; i++){
                $('<option value="'+result.data[i].id+'" data-grade="'+result.data[i].grade+'" >'+result.data[i].name+'</option>').appendTo(self);
                }
            }
		 ...
		 $(document).on('change', '.J_jcate_select',function(){        
            var _this = $(this),
            _pid = _this.val(),
			grade = _this.find("option:selected").attr('data-grade');			
            _this.nextAll('.J_jcate_select').remove();
            if(_pid != ''){
                $.getJSON(uri, {id:_pid,grade:grade}, function(result){
                    if(result.status == '1'){
                        var _childs = $('<select class="J_jcate_select mr10" data-pid="'+_pid+'"><option value="">--'+settings.top_option+'--</option></select>')
                        for(var i=0; i<result.data.length; i++){
                            $('<option value="'+result.data[i].id+'" data-grade="'+result.data[i].grade+'">'+result.data[i].name+'</option>').appendTo(_childs);
                        }
                        _childs.insertAfter(_this);
