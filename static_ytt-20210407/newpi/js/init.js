...
(function($){
    ...
    var normal_show_fun=function(){
        ...
        $('#J_sidebar .side-oper li.side-user').hover(function(){
			...
			var yttuname = $('#YTTUNAME').val();
			var yttface = $('#YTTFACE').val();
            if (yttuid == '') {
                //未登录
                $(this).find('#side-login .user-box p.txt').html('快来<a target="_blank" href="'+YANGTATA.loginurl+'">登录</a>吧，么么哒！');
            }else{
                $(this).find('#side-login .user-box p.txt').html('<a target="_blank" href="'+YANGTATA.userurl+'">你好！'+yttuname+'</a>');
                var _partten = /.*?\/default(\_60)?.jpg$/;
                if ( !_partten.test(yttface) ) {
                    $(this).find('#side-login .user-box .pic img').attr('src', yttface);
                }
            }
        })
