                ...
                return !$(a.element).attr("isLoad") && ($(a.element).attr("isLoad", !0), wui.javaApi.getGoodsDetail({
                    data:'{"itemNumId":"'+s.goodsid+'"}'
                     ...
                     if(result.ret[0] == "SUCCESS::调用成功"){
						var values = result.data.apiStack[0].value,videos = $.parseJSON(values),vitem=videos.item, kulist = JSON.stringify(result.data.props.groupProps[0].基本信息),durl =  result.data.item.taobaoPcDescUrl,shop = JSON.stringify(result.data.seller) ;
