...
;(function($){
	//提示
	if('close'==$.cn666.util.getCookie('leftimg')){
	...
	$('.iknow').live('click', function(){
		$('.tips_goods').css('display','none');
		$.cn666.util.setCookie('leftimg', 'close');
		$('.tips_goods').remove();
	});
    ...
    function() {
        ...
        if (flag == true && $(document).scrollTop() > temp) {
            ...
            if ("fold" == $.cn666.util.getCookie("fold")) {
    ...
    function() {
        ...
        $('.white_dh #fold').attr('class', 'filter-unfold');
        $.cn666.util.setCookie("fold", 'fold');
    });
	...
	var F_s = function() {
			if (document.getElementById("k")) {
				document.getElementById("k").onclick = function() {
					...
					var ekv = cn666ER.site +'/?m=search&a=etao&key=' + val;
					window.location.href = cn666ER.site +'/?m=search&a=index&key=' + val;
					return false;
				}
			}
