...
;(function($){
    $.cn666.user = {
        init: function(options){
            $.cn666.user.bind_form();
			$.cn666.user.sign();
			$.cn666.user.changeTag();
        },
        //登陆页面
        ...
        bind_form: function(){
            if ($('#J_bind_form')) {
                ...
                .ajaxValidator({
                    type: 'get',
                    url: cn666ER.root + '/?m=user&a=ajax_check',
                    data: 'type=email',
                    datatype: 'json',
                    async:'false',
		...
		sign: function(){
			$('.sign_btn').live('click',function(){
				...
				$.getJSON(cn666ER.root + '/?m=sign&a=ajax_sign', function(result){
					if(result.status == 1){
						...
						var dou_calender_tpl='<div class="alert_content"><div class="top_tips">'
											...
											+'<tbody>{TABLE}</tbody>'
											+'</table></div>';
						var not_sign='<p class="tips">恭喜您，成功获得<span class="num">{TODAY_DOU}</span>积分！</p>'
									+'<p class="tom_tips">明天再来，就可以领到 {TOMORROW_DOU} 积分，别忘记哦~</p>';
						var signed='<p class="tips">您今天已经签过到了，明天再来，就可以获得{TOMORROW_DOU}积分！</p>';
