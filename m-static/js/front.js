    ...
    $.cn666.ui = {
        ...
        decode_img: function(context) {
            $('.J_decode_img', context).each(function(){
                ...
                $(this).attr('src', $.cn666.util.base64_decode(uri));  
            });
        }
    },
    $.cn666.tool = {
        //发送邮件
        sendmail: function() {
            return cn666ER.async_sendmail ? ($.get(cn666ER.root + '/?a=send_mail'), !0) : !1;
        },
        //信息提示
        msgtip: function() {
            if(cn666ER.uid){
                ...
                var update = function() {
                    is_update = !0;
                    $.getJSON(cn666ER.root + '/?m=user&a=msgtip', function(result){
                        if(result.status == 1){
