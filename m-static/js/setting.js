...
;(function(){
    $.cn666.setting = {
        init: function(){
            $.cn666.setting.basic();
            $.cn666.setting.password();
            $.cn666.setting.union();
            $.cn666.setting.message_list();
            $.cn666.setting.message();
            $.cn666.setting.target();
        },
        //上传头像
        basic: function(){
            ...
            $('#J_upload_avatar').uploader({
                action_url: cn666ER.root + '/?m=user&a=upload_avatar',
                input_name: 'avatar',
		...
		union: function(){
            $('#links_btn').live('click', function(){
				...
				sibling_input.select();
				 $.cn666.setting.copy(sibling_input.val(),'ok');
            });
			...
			$('input.links_in').blur(function(){
				if($(this).data('focus')){
					 $.cn666.setting.copy($(this).val(),'not');
					$(this).data('focus',0);
				}
			});
        },
        ...
        message: function(){
            $('#J_msg_send').live('click', function(){
                ...
                $.ajax({
                    url: cn666ER.root + '/?m=message&a=publish',
                    type: 'POST',
                    data: {
                        to_id: to_id,
                        content: content
        ...
        message_list: function(){
            ...
            $('#J_msg_list').find('.J_ml_del').live('click', function(){
                ...
                $.getJSON(cn666ER.root + '/?m=message&a=del', {mid:mid}, function(result){
                    if (result.status == '1') {
        ...
        target: function(){
            $('#J_search_target').live('click', function(){
                ...
                $.ajax({
                    url: cn666ER.root + '/?m=message&a=search_target',
                    type: 'POST',
                    data: {
