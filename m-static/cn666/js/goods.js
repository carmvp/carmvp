        ...
        goods_add: function(){
             $("#good_link").focusout(function() {
				  var inform_html = '<li id="detail">' 
					...
					if (link != '') {
						 $.getJSON(cn666ER.root + '/?m=goods&a=ajaxgetid', {url:link}, function(result){
							if(result.status == 1){
								if ($('#detail').size() != 0) {
									$('#detail').remove();
								}
			 ...
			 $("#smt").click(function() {
				 ...
				 var textnum = $.cn666.util.getStrLength(intro);
				 if ($("#good_link").size() > 0) {
					if (good_link == '') {
				...
				$.ajax({
                    url: cn666ER.root + '/?m=goods&a=ajaxadd',
                    type: 'POST',
                    data: {
                        iid: iid,
                        cate_id: cate_id,
			 ...
			 $("#smt_edt").click(function() {
				...
				$.ajax({
                    url: cn666ER.root + '/?m=goods&a=ajaxedit',
                    type: 'POST',
                    data: {
                        iid: iid,
						id: id,
